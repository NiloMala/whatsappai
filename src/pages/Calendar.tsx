...
const formats = {
  // Agenda view date (the per-item date label) - want: "Dom 26/10/25"
  agendaDateFormat: (date: Date, culture: string, localizer: any) => {
    const s = localizer.format(date, "EEE dd/MM/yy", culture);
    return s.charAt(0).toUpperCase() + s.slice(1);
  },
  // Day header (used in day/week views) — show same short format
...
const CalendarPage = () => {
  ...
  useEffect(() => {
    console.log('Injecting calendar styles');
    ...
    style.textContent = `
      ...
      /* Header / day names - reduced height */
      .rbc-header { font-weight: 600 !important; padding: 6px 6px !important; font-size: 13px !important; }
  ...
  /* Center header text (date + weekday) in week/day views */
  ...
  /* Stronger centering for the time/week header table cells: ensure both
     horizontal and vertical centering inside the header row. This targets
     the table-based header used by react-big-calendar's time/week views. */
  ...
  /* Ensure any anchor inside the header fills the cell and centers its text */
  ...
  /* Remove extra top padding/margins that can push content up in some themes */
  .rbc-time-header table thead tr th.rbc-header,
  .rbc-time-header table thead tr th.rbc-header > div,
  .rbc-time-header table thead tr th.rbc-header a {
  ...
  .custom-day-badge { position: absolute; bottom: 4px; left: 50%; transform: translateX(-50%); display: inline-flex; align-items: center; justify-content: center; background-color: #10B981; color: #fff; width: 28px; height: 28px; border-radius: 9999px; font-size: 12px; font-weight: 700; z-index: 10; }
